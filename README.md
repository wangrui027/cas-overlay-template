...
scripts called `Recipes` which may be found inside the `openrewrite` folder of the project.
...
Each recipe is defined in a YAML file and is assigned a name:

```yaml
---
type: specs.openrewrite.org/v1beta/recipe
name: ...
displayName: Upgrade the CAS Overlay to version XYZ
```

To see the list of all recipe names, you can run:

```bash
find ./openrewrite -type f \( -name "*.yaml" -o -name "*.yml" \) -exec grep -m 1 "name:" {} \; | sed -e 's/.*name: *//' -e 's/^"//' -e 's/"$//'
```
...
```bash
./gradlew rewriteDryRun -DactiveRecipe=[recipe name] --no-configuration-cache
```

When you are ready, you can run the actual recipe:

```bash
./gradlew rewriteRun -DactiveRecipe=[recipe name] --no-configuration-cache
```
